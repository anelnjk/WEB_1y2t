...
long_haired.onclick = () =>{
    choosingData = "long-haired-dogs";
}
shorthaired.onclick = () =>{
    choosingData = "shorthaired-dogs";
}
...
const showInfo = (choice) =>{
    $.getJSON("https://my-json-server.typicode.com/anelnjk/mockjson/"+choosingData+'/', function(data){
        ...
        for(i=0; i<num;i++)
      {
        ...
        $('#p1')[0].innerHTML = "Specie name: " + JSON.stringify(data[id_dog].name);
        $('#p2')[0].innerHTML = "Price: " + JSON.stringify(data[id_dog].price)+ "$" ;
        $("#dog-img").attr("src",data[id_dog].image);
    });
}
